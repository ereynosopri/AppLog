...
export default function Home() {
  ...
  async function testSupabaseConnection() {
    const res = await supabase.from('teammates').select('id, name').limit(1);
    if (res.error) throw new Error('Supabase test failed: ' + res.error.message);
    notify('Supabase connection works. Teammates table returned ' + (res.data?.length || 0) + ' row(s).');
  }
  ...
  return <main className="app">
    ...
    <section className={tab==='log'?'section active card':'section card'}>
      <form onSubmit={submitAction(form.id ? 'Updating appointment' : 'Saving appointment', saveAppointment)}>
        ...
        <div className="row"><button type="submit" className="btn primary">{form.id?'Update Appointment':'Save Appointment'}</button>{form.id && <button type="button" className="btn secondary" onClick={buttonAction('Canceling edit', () => setForm(blankFormFor(user)))}>Cancel Edit</button>}</div>
      </form>
    </section>
    ...
    <section className={tab==='settings'?'section active':'section'}><div className="card"><h2>Team Members</h2><p className="status"><strong>Action Status:</strong> {statusMessage}</p><div className="row"><button type="button" className="btn secondary" onClick={buttonAction('Testing Supabase', testSupabaseConnection)}>Test Supabase Connection</button></div><form onSubmit={submitAction('Adding teammate', addTeammate)}><div className="grid2"><input placeholder="Name" value={newMate.name} onChange={e=>setNewMate({...newMate,name:e.target.value})}/><input placeholder="Rep ID" value={newMate.rep_id} onChange={e=>setNewMate({...newMate,rep_id:e.target.value})}/><input placeholder="Phone" value={newMate.phone} onChange={e=>setNewMate({...newMate,phone:e.target.value})}/><input placeholder="Email" value={newMate.email} onChange={e=>setNewMate({...newMate,email:e.target.value})}/></div><button type="button" className="btn primary full" onClick={buttonAction('Adding teammate', addTeammate)}>Add Member</button></form>{teammates.map(t=><div className="reportrow" key={t.id}><strong>{t.name}</strong> • Rep ID: {t.rep_id||'—'} • {t.email||'No email'} <div className="row">{t.is_admin?<button type="button" className="btn secondary" onClick={buttonAction('Removing admin', () => removeAdmin(t))}>Remove Admin</button>:<button type="button" className="btn secondary" onClick={buttonAction('Promoting admin', () => promote(t))}>Promote to Co-Admin</button>}<button type="button" className="danger" onClick={buttonAction('Deactivating teammate', () => deactivate(t))}>Deactivate</button></div></div>)}</div>
