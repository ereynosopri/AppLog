...
export default function AppointmentLogApp() {
  ...
  useEffect(() => {
    if (!currentUser || form.id) return;
    if (currentUser.role === 'teammate' && form.member !== currentUser.name) {
      setForm(f => ({ ...f, member: currentUser.name }));
    }
  }, [currentUser, form.id, form.member]);
  ...
  function saveAppointment() {
    ...
    const effectiveMember = isAdmin ? form.member : (currentUser?.name || form.member);
    const missingFields = [];
    if (!effectiveMember) missingFields.push('Team Member Name');
    if (!form.date) missingFields.push('Date');
    if (!form.clientName.trim()) missingFields.push('Prospect / Client Name');
    if (!appointmentType) missingFields.push('Appointment Type');
    if (!source) missingFields.push('Source');
    if (!form.outcome) missingFields.push('Outcome');
    if (missingFields.length) return flash('Missing: ' + missingFields.join(', '));
    ...
    const entry: Entry = { id: form.id || Date.now(), member: effectiveMember, date: form.date, time: form.time, clientName: form.clientName.trim(), appointmentType, source, lessons: form.lessons.trim(), outcome: form.outcome, detail, weekKey: weekKey(form.date) };
  ...
  return <div className="app">
    ...
    {tab === 'log' && <section className="section active card">
      <label>Team Member Name</label><select value={isAdmin ? form.member : currentUser.name} disabled={!isAdmin} onChange={e=>setForm({...form,member:e.target.value})}><option value="">Select team member...</option>{(members.some(m=>m.name===currentUser.name)?members:[...members,{name:currentUser.name,repId:'',phone:'',email:''}]).map(m=><option key={m.name}>{m.name}</option>)}</select>
